...
description: "Recover S05 Stage-2 memory isolation candidates from accepted S03 and S04 ARM64 EL2 evidence. Use when the workflow needs VTCR/VTTBR/VMID/page-table descriptor/map-unmap/fault/ownership evidence without importing Linux, KVM, Xen, or Hafnium implementation details."
---
...
1. Enforce upstream gates.
   ...
   - If upstream is not accepted, emit blocked/forward-test status and do not produce S06-consumable ownership.
   - Exclude S03 blocking unresolved blob ranges from page-table root or descriptor claims.
...
5. Recover memory ownership candidates.
   ...
   - Do not mark S06-ready ownership from Stage-2 sysreg anchors alone; descriptor layout and page/resource ownership must be integrated first.
   ...
   - Record caller argument roots and local field offsets. Stable offsets across switch/restore paths may become type seeds, but still need owner and lifecycle proof before S06 production.
   ...
   - If lifecycle edges strengthen confidence but VMID, owner object, or teardown path is still missing, keep S05 in review-required state and block S06 production.
   ...
   - If no object-like root remains after classification, emit a blocking Unknown for Stage-2 ownership closure and leave S06 blocked.
   ...
   - If teardown-like candidates are not connected to the same owner object, emit partial cleanup evidence and keep S06 blocked.
...
- `S05/records/recover-hypervisor-stage2-memory-model.evidence.jsonl`
- `S05/records/recover-hypervisor-stage2-memory-model.decisions.jsonl`
- `S05/records/recover-hypervisor-stage2-memory-model.unknowns.jsonl`

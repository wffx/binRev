#!/usr/bin/env python3
"""Generate S07 review-seed security lifecycle artifacts.

This pass is intentionally conservative. It consumes S05/S06 review-seed
...
def load_inputs() -> dict[str, Any]:
    return {
        ...
        "s06_manifest": read_json(S06 / "stage-manifest.json"),
        "s06_service": read_json(S06 / "service-model.json"),
        "s06_scheduler": read_json(S06 / "scheduler-model.json"),
        "s06_interrupt": read_json(S06 / "interrupt-model.json"),
        "s06_vm_config": read_json(S06 / "vm-config-model.json"),
        "s06_states": read_jsonl(S06 / "state-machines.jsonl"),
    }


def build_lifecycle_model(inputs: dict[str, Any]) -> dict[str, Any]:
    state_hypotheses = inputs["s06_states"]
    review_sequence = []
...
def build_resource_transitions(inputs: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    service_links = inputs["s06_service"].get("review_seed_links", [])
    for idx, link in enumerate(service_links, 1):
        transitions.append(
            {
                "id": f"S07-RES-HYP-{idx:04d}",
                "status": "model_hypothesis",
...
def build_security_lifecycle(
    ...
    return {
        ...
        "upstream": {
            ...
            "S06_status": inputs["s06_manifest"].get("status"),
            "S06_production_readiness": inputs["s06_service"].get("s07_readiness", {}).get("production"),
            "S06_review_seed_readiness": inputs["s06_service"].get("s07_readiness", {}).get("review_seed"),
        },
...
def build_manifest(security_model: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "s08_readiness": security_model["s08_readiness"],
...
def build_ida_proposal() -> dict[str, Any]:
    return {
        ...
        "actions": [],
        "reason": (
            "S07-RW1 has only lifecycle/security hypotheses and unknowns. It does not propose function names, "
...
def main() -> None:
    ...
    write_json(S07 / "lifecycle-model.json", lifecycle_model)
    write_json(S07 / "vm-lifecycle-model.json", lifecycle_model)
    write_json(S07 / "hkip-model.json", hkip_model)
    write_jsonl(S07 / "resource-transitions.jsonl", resource_transitions)
    write_jsonl(
        S07 / "state-transitions.jsonl",
        lifecycle_model["review_seed_lifecycle_hypotheses"],
    )
    write_json(S07 / "security-lifecycle-model.json", security_model)
    write_json(S07 / "stage-manifest.json", manifest)
    write_json(S07 / "artifact-validation-rw1.json", validation)
    write_json(S07 / "ida-change-proposal-rw1.json", ida_proposal)
    ...
    evidence_rows = [
        {
            ...
            "summary": "S06 production is blocked but review-seed readiness permits hypothesis-only S07 continuation.",
        },
        {
            "id": "E-S07-RW1-0002",
            "kind": "lifecycle_seed",
    ...
    decision_rows = [
        {
            "id": "D-S07-RW1-0001",
            "decision": "run_s07_in_review_seed_mode",
        ...
        json.dumps(
            {
                ...
                "s08_readiness": manifest["s08_readiness"],
            },
            ensure_ascii=False,
            sort_keys=True,
        )

...
3. Copy/freeze repository, reports, source maps, unresolved indexes, and audit artifacts.
   - In workflow v2, copy the canonical source repository to `S10/deliverable/source/recovered-hypervisor/`.
   - In workflow v2, copy JSON/JSONL evidence only to `S10/deliverable/evidence/`, never inside the source payload.
   - Include S06 type/offset/global review-seed evidence under `S10/deliverable/evidence/S06/` when present.
   - Include S08 presentation and planning evidence such as global aliases, offset-field annotations, externalized lifted pseudocode reviews, diagnostic-summary candidate indexes, candidate-driven names, simple-pseudocode names, module rehome indexes, source-symbol repair indexes, source-symbol repair normalization indexes, duplicate-wrong-file cleanup indexes, and source-class synchronization indexes under `S10/deliverable/evidence/S08/` when present.
...
6. In workflow v2, run `scripts/package_v2_source_delivery.py --case-id <case-id>`.
   ...
   - Emit a top-level `S10/deliverable/README.md` that explains source/evidence/report boundaries and repeats the S09 readability gates.

...
- **Stage**：S03
- **角色**：Worker
- **描述**：从 IDA xref、访问宽度、固定 stride、字符串、常量和可写状态访问中恢复数组、表、字符串、指针表和状态对象候选；用于识别 code/data 冲突和 unresolved region。
- **目标**：建立与函数候选互补的数据对象模型，并保护 literal pool、inline data 和表结构不被错误代码化。
...
- **Stage**：S03
- **角色**：Integrator
- **描述**：合并函数、数据对象和间接控制流候选，解决函数重叠、code/data 冲突和调用图不一致；用于生成唯一 program model、直接调用图和 IDA change proposal。
- **目标**：形成下游架构分析可稳定消费的统一程序结构模型。
...
当前 39 个条目均为 `specified`，尚未在 `skills/` 下创建正式 Skill 包。

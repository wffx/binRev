...
| 4 | `apply-reviewed-ida-changes` | S01–S06 | IDA Executor |
| 5 | `initialize-hypervisor-recovery-case` | S00 | Worker |
| 6 | `enforce-recovery-constraints` | S00 | Worker |
| 7 | `analyze-arm64-image-layout` | S00 | Worker |
| 8 | `classify-binary-regions` | S00 | Worker |
| 9 | `prepare-ida-image-database` | S01 | Worker |
| 10 | `resolve-arm64-load-address` | S01 | Worker |
| 11 | `snapshot-ida-analysis-state` | S01–S06 | Worker |
| 12 | `recover-ida-functions` | S02 | Worker |
| 13 | `recover-binary-data-objects` | S02 | Worker |
| 14 | `recover-indirect-control-flow` | S02 | Worker |
| 15 | `integrate-program-structure` | S02 | Integrator |
| 16 | `recover-arm64-boot-flow` | S03 | Worker |
| 17 | `recover-arm64-exception-model` | S03 | Worker |
| 18 | `recover-arm64-context-layout` | S03 | Worker |
| 19 | `recover-el2-architecture-semantics` | S03 | Worker |
| 20 | `integrate-el2-architecture-model` | S03 | Integrator |
| 21 | `recover-hypervisor-cpu-vcpu-model` | S04 | Worker |
| 22 | `recover-hypervisor-stage2-memory-model` | S04 | Worker |
| 23 | `integrate-hypervisor-runtime-model` | S04 | Integrator |
| 24 | `recover-hypervisor-vm-config` | S05 | Worker |
| 25 | `recover-hypervisor-scheduler` | S05 | Worker |
| 26 | `recover-hypervisor-interrupt-routing` | S05 | Worker |
| 27 | `integrate-hypervisor-service-model` | S05 | Integrator |
| 28 | `recover-hypervisor-vm-lifecycle` | S06 | Worker |
| 29 | `recover-hypervisor-hkip-model` | S06 | Worker |
| 30 | `integrate-hypervisor-security-lifecycle` | S06 | Integrator |
| 31 | `synthesize-hypervisor-repository` | S07 | Synthesizer |
| 32 | `generate-recovery-source-map` | S07 | Synthesizer |
| 33 | `index-recovery-evidence` | S07 | Synthesizer |
| 34 | `audit-recovery-static-consistency` | S08 | Auditor |
| 35 | `audit-hypervisor-security-invariants` | S08 | Auditor |
| 36 | `assess-recovery-coverage` | S08 | Auditor |
| 37 | `integrate-recovery-audit` | S08 | Integrator |
| 38 | `write-hypervisor-recovery-report` | S09 | Delivery |
| 39 | `package-hypervisor-recovery-delivery` | S09 | Delivery |
...
- **目标**：使 S00–S09 严格按照 Stage Contract 推进，并保证每次状态迁移都有可审计的 Artifact、Review 和 Decision。
...
- **Stage**：S00
- **角色**：Worker
- **描述**：根据固定边界生成并检查 Case constraint profile；用于声明唯一输入、IDA-only、静态分析、禁止外部源码/符号/日志/平台资料，以及不可宣称可编译、行为等价或安全证明。同时扫描本地工具链（IDA 安装路径、IDA MCP 连通性、Hex-Rays 可用性），生成 tool-status.json。
- **目标**：把项目边界转换为机器可读约束，并验证下游 Stage 所需工具环境就绪，防止下游 Skill 引入未授权输入、工具或结论。
...
- **Stage**：S01
- **角色**：Worker
- **描述**：按照 S00 region map 将唯一 `Image` 装载到 IDA AArch64 little-endian 数据库；用于设置中性 segment、processor 和初始 entry candidate，不写入业务名称或高级类型。
...
- **Stage**：S02
- **角色**：Worker
- **描述**：按 text code-first 策略恢复 S02 数据对象与 code/data boundary：主 `.text` 中间优先还原为 ARM64 code，只有 binary header/tail 或明确非 text 区才允许保留外部数据；不可解码 word 必须进入 `.inst`/`instruction_fallback` blocker。
- **目标**：建立与函数候选互补的数据对象模型，并在进入 S03 前完成全量 code/data/data-island 结构修复；主 `.text` 中间 qword/DCQ 或未审核 `.inst fallback` 必须阻塞或 rework。
...
- **目标**：形成下游架构分析可稳定消费的统一程序结构模型，并保证 S03 输入没有结构性未识别 bit/data 区域。

## 9. S03 ARM64 EL2 架构语义
...
- **目标**：形成 S04 可消费的统一 ARM64 EL2 架构语义模型。
...
- **目标**：形成 S07 合成与 S08 安全审计所需的统一资源和权限生命周期模型。
...
- **Stage**：S07
- **角色**：Synthesizer
- **描述**：只依据 S02–S06 accepted 模型合成模块目录、C-like/C 候选、类型定义和符号化 AArch64 `.S`；不重新分析汇编或补写模型外业务逻辑。
...
- **Stage**：S09
- **角色**：Delivery
- **描述**：仅根据 S00–S08 accepted Artifact 编写最终恢复报告；用于汇总模型、覆盖率、审计发现、限制、未知项和固定约束，不产生新的技术推断。
...
| G02-A | S00–S01 基础闭环 Skills | `accepted` | `initialize-hypervisor-recovery-case`、`enforce-recovery-constraints`、`analyze-arm64-image-layout`、`classify-binary-regions`、`prepare-ida-image-database`、`resolve-arm64-load-address`、`snapshot-ida-analysis-state` |
| G02-B | S02 程序结构 Skills | `rework`（forward-test 发现需优化；不代表生产 Stage 引入符号 oracle） | `recover-ida-functions`、`recover-binary-data-objects`、`recover-indirect-control-flow`、`integrate-program-structure` |
| G02-C | S03 EL2 架构语义 Skills | `forward_test_deferred_by_s02_rework`（boot-slice forward-test 已完成；生产 S03 依赖 S02 accepted，当前 S02-RW4 仍有 12 个 blocking unresolved code/data blob） | `recover-arm64-boot-flow`、`recover-arm64-exception-model`、`recover-arm64-context-layout`、`recover-el2-architecture-semantics`、`integrate-el2-architecture-model` |
| G03 | S04–S06 领域 Skills | `created_blocked_by_upstream`（S04–S06 domain skills 已创建；实测需等待 S02/S03/S04 accepted） | `recover-hypervisor-cpu-vcpu-model`、`recover-hypervisor-stage2-memory-model`、`integrate-hypervisor-runtime-model`、`recover-hypervisor-vm-config`、`recover-hypervisor-scheduler`、`recover-hypervisor-interrupt-routing`、`integrate-hypervisor-service-model`、`recover-hypervisor-vm-lifecycle`、`recover-hypervisor-hkip-model`、`integrate-hypervisor-security-lifecycle` |
| G04 | S07–S09 合成、审计与交付 Skills | `created_blocked_by_upstream`（S07-S09 skills 已创建；实测需等待 S02-S06 accepted） | `synthesize-hypervisor-repository`、`generate-recovery-source-map`、`index-recovery-evidence`、`check-recovered-code-consistency`、`check-hypervisor-security-invariants`、`compute-recovery-coverage`、`integrate-static-audit-report`、`generate-final-recovery-report`、`package-recovery-deliverable` |

...
|---|---|
| S00 | 不使用 IDA。 |
| S01 | 建立或读取中性 IDA baseline，确认 processor、endianness、entry、segment、analysis status。 |
| S02 | 基于 accepted S01 checkpoint 恢复函数、数据对象和间接控制流候选。 |
| S03 | 基于 accepted S02 checkpoint 恢复 ARM64/EL2 架构语义候选。 |
| S04 | 写入已审核 CPU/vCPU/Stage-2 类型和名称。 |
| S05 | 写入已审核 VM config、scheduler、interrupt 模型。 |
| S06 | 写入已审核 lifecycle/HKIP 模型。 |
| S07 | 只读 accepted IDA checkpoint，用于代码仓合成。 |
| S08 | 只读 accepted IDA checkpoint，用于一致性和安全审计。 |
| S09 | 冻结 final IDA checkpoint。 |
...
因此它只能验证 S01 的 IDA MCP 通路、metadata 读取、函数列表和反汇编读取；不能验证 ARM64 Image header、EL2 system register 或 ARM64 load-base 推断。

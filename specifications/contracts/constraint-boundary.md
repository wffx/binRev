...
| Stage 是否有显式输入输出 | S00-S10 必须定义输入、输出和退出条件 | 符合 |
...
- 当 S01/S02 Skill 对该样本输出 `incompatible` 或 `not_applicable` 时，这是正确的边界行为。

`tests/xen_arm64` / `tests/xen_arm64.i64` 可以用于验证目标格式 ARM64 Image 的 S00-S03 小范围闭环。
